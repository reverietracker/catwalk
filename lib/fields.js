const { capitalize, smushName } = require('./string');
...
class BaseField {
    constructor(name, opts) {
        this.opts = opts || {};
        this.name = name;
        this.label = this.opts.label || capitalize(name);
    }
...
class ListField extends BaseField {
    constructor(name, subfield, opts) {
        super(name, opts);

        this.subfield = subfield;

        this.startIndex = this.opts.startIndex || 0;
        if ('endIndex' in this.opts) {
            this.endIndex = this.opts.endIndex;
            this.length = this.endIndex - this.startIndex;
        } else if ('length' in this.opts) {
    ...
    getDefault() {
        if ('default' in this.opts) {
            return this.clean(this.opts.default);
        } else {
