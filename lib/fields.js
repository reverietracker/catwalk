import { camelCaseToLabel, smushName } from './string.js';
...
class ValueField extends BaseField {
    ...
    getDefault() {
        return ('default' in this.options) ? this.clean(this.options.default) : null;
    }
...
class EnumField extends ValueField {
    constructor(name, options) {
        super(name, options);
        this.choices = this.options.choices;

        // dict keys are cast to string, so keep a lookup to translate them back
        // to their original type
        this.typedValuesByChoice = {};
        for (const [choice, label] of this.choices) {
            this.typedValuesByChoice[choice] = choice;
        }
    }
    clean(val) {
        if (!(val in this.typedValuesByChoice)) throw new ValidationError();
        return this.typedValuesByChoice[val];
    }
}
...
class ListField extends BaseField {
    ...
    getDefault() {
        if ('default' in this.options) {
            ...
            const arr = new Array(this.endIndex);
...
export {
    BaseField, ValueField, NumberField, IntegerField, BooleanField, EnumField, ModelField, ListField, TupleField, StructField
};
