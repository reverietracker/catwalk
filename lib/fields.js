...
class ModelField extends ValueField {
    ...
    serialize(instance, data) {
        data[this.name] = instance[this.internalName].toData();
    }
    deserialize(val) {
...
class ListField extends BaseField {
    ...
    serialize(instance, data) {
        data[this.name] = instance[this.internalName].map(item => this.subfield.serialize(item));
    }
    deserialize(val) {
...
class BaseStructField extends BaseField {
    ...
    get(obj, keys) {
        ...
        let val = obj[key];
        if (rest.length) {
            return this.fieldLookup[key].get(val, rest);
        } else {
            return val;
        }
    }
...
class TupleField extends BaseStructField {
    ...
    init(instance, data) {
        if (this.name in data) {
            const sourceArray = data[this.name];
            instance[this.name] = this.fieldList.map((field, i) => {
                try {
...
class StructField extends BaseStructField {
    ...
    init(instance, data) {
        if (this.name in data) {
            const sourceObj = data[this.name];
            const obj = {};
            this.fieldList.forEach((field) => {
                if (field.name in sourceObj) {
                    try {
                        obj[field.name] = field.clean(sourceObj[field.name]);
                    } catch (e) {
                        if (e instanceof ValidationError) {
                            obj[field.name] = field.getDefault();
                        } else {
                            throw e;
                        }
                    }
